...
export const PanditCard: React.FC<PanditCardProps> = ({ pandit }) => {
  ...
  return (
    <div className="bg-white rounded-2xl sm:rounded-3xl border border-[#eadfce] overflow-hidden shadow-spiritual hover:shadow-spiritual-hover transition-all duration-300 flex flex-col group h-full">
      {/* Image & Badge */}
      <div className="relative h-44 sm:h-56 lg:h-60 w-full overflow-hidden bg-amber-950/10 shrink-0">
        <Image
          src={imgSrc}
          alt={pandit.name}
          fill
        ...
        {pandit.isVerified && (
          <div className="absolute top-2 right-2 sm:top-3 sm:right-3 bg-glass border border-amber-300 text-[#8f3f12] text-[9px] sm:text-[11px] font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full flex items-center gap-0.5 shadow-sm">
        ...
        <div className="absolute bottom-2 left-2 right-2 sm:bottom-3 sm:left-3 sm:right-3 flex items-center justify-between text-white text-[9px] sm:text-xs font-semibold">
          <div className="flex items-center gap-0.5 sm:gap-1 bg-black/60 px-1.5 py-0.5 sm:px-2 sm:py-0.5 rounded-full backdrop-blur-sm">
            <Award className="w-3 h-3 text-amber-400" />
            <span>{pandit.experience} Yrs</span>
          </div>
          <div className="flex items-center gap-0.5 sm:gap-1 bg-amber-500/90 text-white px-1.5 py-0.5 sm:px-2 sm:py-0.5 rounded-full font-bold">
      ...
      {/* Content */}
      <div className="p-3 sm:p-4 lg:p-5 flex-1 flex flex-col justify-between space-y-2 sm:space-y-3">
        <div>
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {pandit.specializations.slice(0, 2).map((spec, i) => (
              <span
                key={i}
                className="text-[9px] sm:text-[10px] font-medium bg-[#fffaf2] text-[#8f3f12] border border-[#eadfce] px-1.5 py-0.5 rounded-md truncate max-w-full"
              >
                {spec}
              </span>
            ))}
        ...
        <div className="pt-2 border-t border-[#eadfce] grid grid-cols-2 gap-1.5 sm:gap-2">
          <Link
            href={`/pandit/${pandit.slug}`}
            className="text-center text-[10px] sm:text-xs font-semibold py-1.5 sm:py-2 px-1 rounded-lg sm:rounded-xl border border-[#c96b18] text-[#c96b18] hover:bg-[#c96b18]/10 transition-colors truncate"
          >
            Profile
          </Link>
          <a
            ...
            className="text-center text-[10px] sm:text-xs font-bold py-1.5 sm:py-2 px-1 rounded-lg sm:rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm transition-all flex items-center justify-center gap-0.5 truncate"
          >
            <span>WhatsApp</span>
